...
$E(\widehat{b} - b)A(\widehat{b} - b) = \displaystyle\sum_{i, j}a_{ij}E(\widehat{b_i} - b_i)(\widehat{b_j} - b_j) = \displaystyle\sum_{i, j}a_{ij}\Cov(\widehat{b_i}, \widehat{b_j}) = \sigma^2 \displaystyle\sum_{i, j}a_{ij}a_{ij}^{-1} = \sigma^2 \displaystyle\sum_{i} \displaystyle_{j}a_{ij}a_{ji}^{-1} = \sigma^2 \displaystyle_{i = 1}^m 1 = \sigma^2 m$\\
...
n = 1$\\
% $while true {\\
% if \frac{L_{0n}}{L_{1n}} \leq A_0 \text{ then return reject H_0}\\
% if \frac{L_{0n}}{L_{1n}} \geq A_1 \text{ then return accept H_0}\\
% ++n
% }$
...
\\
\bf{Теорема}\\
Пусть $\nu$ - количество итераций $\Rto P(@n \geq n | H_0) \to 0, P(\nu \geq n | H_1 \to 0), n \to \infty$\\
\bf{Доказательство}\\
$r - fixed$\\
$\eta_1 = Z_1 + \dots + Z_r$\\
...
$1 = \displaystyle\sum_{n = 1}^{\infty}P(\nu = n | H_0) = \displaystyle\sum_{n = 1}^{\infty}P(\text{accept at the step n | }H_0) + \displaystyle\sum_{n = 1}^{\infty}P(\text{reject at the step n | }H_0) = \alpha = \displaystyle\sum{n = 1}^{\infty}P(\text{reject at the step n |}H_0) \leq \frac{1}{A_1}\displaystyle\sum_{n = 1}^{\infty}P(\text{reject at the step n | }H_1)$\\
